...
for movie in movie_df.values:
    ...
    cols = st.columns(12)
    cols[0].write(movie_details[0])
    response = requests.get(movie_details[1])
    cols[1].image(response.content, use_column_width='auto')
    cols[2].write(movie_details[2])
    cols[3].write(movie_details[3])
    response = requests.get(f'https://countryflagsapi.com/png/{movie_details[4].lower()}')
    cols[4].image(response.content, width=30)
    cols[5].write(movie_details[5])
    cols[6].write(movie_details[6])
    try:
        cols[7].write(int(movie_details[7]) / 100)
        cols[8].write(int(movie_details[8]) / 100)
        cols[9].write(int(movie_details[9]) / 100)
        cols[10].write(int(movie_details[10]) / 100)
        cols[11].write(int(movie_details[11]) / 100)
    except:
        pass
